...
class HTMLParserTests {
    ...
    void preventsInsertionOnEitherSideOfMatchingElement() {
        Document beforeExisting = Jsoup.parseBodyFragment(
                "<p>First</p><pre>Code</pre><p>Last</p>");
        Document afterExisting = Jsoup.parseBodyFragment(
                "<p>First</p><pre>Code</pre><p>Last</p>");

        boolean insertedBefore = HTMLParser.insertAtRandomPosition(
                beforeExisting.body(), new Element("pre"),
                randomReturning(1), true);
        boolean insertedAfter = HTMLParser.insertAtRandomPosition(
                afterExisting.body(), new Element("pre"),
