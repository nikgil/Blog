...
<#-- Boost tag links to replace blog content without scrolling, carry tag search/page state, and cancel older filter requests. -->
...
<nav class="tag-filter__pagination" aria-label="Tag pages">
  <button id="tag-page-previous" class="button is-small tag-filter__page-link" type="submit" name="page"
    value="${currentTagPage - 1}" aria-label="Previous 10 tags" <#if !(hasPreviousTagPage!false)>disabled</#if>>
  ...
  <button id="tag-page-next" class="button is-small tag-filter__page-link" type="submit" name="page"
    value="${currentTagPage + 1}" aria-label="Next 10 tags" <#if !(hasNextTagPage!false)>disabled</#if>>
    <span aria-hidden="true">→</span>
  </button>
</nav>
