      ...
      <#list tags as tag>
        <#if tag_index lt 10>
          <li class="tag-filter__item">
            <a class="tag tag-filter__link" href="?tag=${tag.slug?url('UTF-8')}">
...
<#assign currentTagPage=tagPage!0>
  <nav class="tag-filter__pagination" aria-label="Tag pages">
    ...
    <#if hasNextTagPage!false>
      ...
      <#else>
        <button id="tag-page-next" class="button is-small tag-filter__page-link" type="button" aria-label="Next 10 tags"
          disabled>
          <span aria-hidden="true">→</span>
        </button>
    </#if>
